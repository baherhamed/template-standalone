...
export class MessageComponent implements OnChanges {
  ...
  async ngOnChanges() {
    this.systemMessage.titleClass = this.systemMessage.titleClass
      ? this.systemMessage.titleClass
      : 'model-header';
    await this.displayMessageLanguage(this.systemMessage.message);
    this.dialog.showDialog(
      site.dialogNames.showMessage,
  ...
  async displayMessageLanguage(message: systemLanguages) {
    ...
    this.displayMessage =
      language && language === site.language.ar ? message.ar : message.en;
  }
